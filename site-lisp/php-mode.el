...
;; Based on php-mode 1.2.0 (http://php-mode.sourceforge.net/) by Turadg Aleahmad
;; Copyright (C) 1999-2004 Turadg Aleahmad      <turadg at users.sourceforge.net>
;; Copyright (C) 2007      Troels Knak-Nielsen  <swaxi at users.sourceforge.net>

;; Maintainer: Troels Knak-Nielsen
...
;; PHP mode is a major mode for editing PHP 3, 4 and 5 source code.  It's
...
;; Ville Skytta, Giacomo Tesio, Troels Knak-Nielsen

;;; Changelog:

;; 1.3
;;   Fixed various font-locking bugs (Troels Knak-Nielsen)
;;   Added some PHP5 specific keywords (Troels Knak-Nielsen)
...
(if (and php-speedbar-config (symbolp 'speedbar))
    (speedbar-add-supported-extension "\\.\\(inc\\|php[s345]?\\)"))
...
;; hook for replacing browse-url
(defvar php-browse-url 'browse-url)
(defun php-browse-url (url)
  (funcall php-browse-url url))

;; Define function documentation function
;; (defun php-search-documentation ()
;;   "Search PHP documentation for the word at the point."
;;   (interactive)
;;   (php-browse-url (concat php-search-url (current-word t) "#content")))
(defun php-search-documentation ()
  "Open a window showing the PHP documentation for the word under the point"
  (interactive)
  (php-browse-url
   (concat "http:///docs.php.net/search.php"
           (format "?pattern=%s&show=quickref"
...
(defconst php-font-lock-keywords-1
  (list
   ;; Fontify constants
   ;; We need 4 permutations, to account for end-of-line as delimiter ... This is seriously hacky
   (cons
   ...
   ;; Fontify keywords
   ;; same story -- this could be done smarter ...
   (cons
    (concat "^\\(" php-keywords "\\)[ \t;{]+")
...
(defconst php-font-lock-keywords-2
  (append
   ...
   (list
    ...
    ;; class declaration
    '("^[ \t]*\\(abstract\\)?[ \t]*\\(class\\|class\\|interface\\)[ \t]+\\(\\(?:\\sw\\|\\s_\\)+\\)?"
      (2 font-lock-keyword-face) (3 font-lock-type-face nil t))
